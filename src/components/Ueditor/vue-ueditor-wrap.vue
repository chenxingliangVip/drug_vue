...
export default {
  ...
  methods: {
    ...
    // 实例化编辑器
    _initEditor() {
      this.$refs.script.id = this.id =
        'editor_' +
        Math.random()
          .toString(16)
          .slice(-6) // 这么做是为了支持 Vue SSR，因为如果把 id 属性放在 data 里会导致服务端和客户端分别计算该属性的值，而造成 id 不匹配无法初始化的 BUG
      this.init()
    ...
    _loadConfig() {
      return new Promise((resolve, reject) => {
        ...
        configScript.src =
          this.mixedConfig.UEDITOR_HOME_URL + 'ueditor.config.js'
    ...
    _loadCore() {
      return new Promise((resolve, reject) => {
        ...
        coreScript.src =
          this.mixedConfig.UEDITOR_HOME_URL + 'ueditor.all.min.js'
